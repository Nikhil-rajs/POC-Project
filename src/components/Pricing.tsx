          ...
          {fields.pricingCards.map((card: pricingCard, index: number) => {
            return (
              <div className='card-content rounded-md ' key={index}>
                <h1 className="card-version text-[22px] font-light tracking-widest text-[#0095f7] mb-6 md:text-[17px] lg:text-[22px]"><Text field={card.fields.pricingVersion}/></h1>
                <p className="card-cost text-[60px] font-bold text-[#3a3f52] mb-0"><Text field={card.fields.pricingCost}/></p>
                <p className="card-period text-[12.8px] text-[#8da2b5] font-normal tracking-wide"><Text field={card.fields.pricingPeriod}/></p>
                <p className="card-para tracking-wider font-normal text-[17px] text-left my-3.5 text-[#53627C]  md:text-[14px] lg:text-[17px]"><Text field={card.fields.pricingPara}/></p>
                    ...
                    return(
                      <div className='' key={index}>
                       <li className='list-none ' > 
                        <ul className='leading-relaxed'><FontAwesomeIcon icon={faCheckCircle} className='mr-2.5 text-xl'/><span className='text-[14px] text-[#8da2b5] font-light tracking-widest'><Text field={services.fields.serviceTitle}/></span></ul>                 
                        </li>
                       </div>
                    );
                  })}
                </div>
                <div className="card-cta text-center text-white tracking-widest bg-[#6772e5] mt-5"><Link field={card.fields.pricingCTA}/>read more</div>
              </div>
            );
          })}
        </div>
